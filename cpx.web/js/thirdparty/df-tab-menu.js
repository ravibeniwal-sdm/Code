			...
			compile: function($element, $attrs) {
				...
				return function ($scope, $element, $attrs) {
					...
					var getElementsSize = function() {
						var elements = root.querySelectorAll('li[data-menu-item][role=presentation][available=true]');
					...
					var getVisibleItems = function(_maxWidth, _activeItemIndex) {
						...
						var items = root.querySelectorAll('li[data-menu-item][role=presentation][available=true]');
					...
					var buildMenu = function() {
						...
						if(visibleItems.length < root.querySelectorAll('li[data-menu-item][role=presentation][available=true]').length) {
							...
							var nelements = root.querySelectorAll('li[data-menu-item][role=presentation][available=false]');
							angular.element(nelements).addClass('ng-hide').attr('aria-hidden','true');
							
							var all_elements = root.querySelectorAll('li[data-menu-item][role=presentation]');
							
							for(var i = 0; i < all_elements.length; i++) {
								if(angular.element(all_elements[i]).attr('available')=='true'){
									//if in visible items hide it else show it data-menu-item
									var isvisible=false;
									
									for(var x=0; x<visibleItems.length; x++){
										if(angular.element(all_elements[i]).attr('data-menu-item') == angular.element(elements[visibleItems[x]]).attr('data-menu-item')){
											isvisible=true;
											break;
										}
									}
